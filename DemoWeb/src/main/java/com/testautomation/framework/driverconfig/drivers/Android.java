...
public class Android extends BaseMobileDriver<AppiumDriver, DesiredCapabilities, Android> {
    ...
    public Android(DataConfig dataConfig) {
    ...
    public DesiredCapabilities getDefaultOptions() {
        ...
        capabilities.setCapability("appPackage", dataConfig.testAppName);
        capabilities.setCapability("appActivity", dataConfig.mb_appActivity);
      ...
      throws MalformedURLException {
          if (StringUtils.equalsIgnoreCase(dataConfig.testNetowk, "cloud")) {
              if (StringUtils.equalsIgnoreCase(Prop.getProperty("mobile.cloud.env"), "public")) {
                  dataConfig.androidDriver = setMobileAppDriverManage(new AndroidDriver(new URL(Prop.getProperty("mobile.cloud.public.url")), getOptions(options)));
              } else {
                  dataConfig.androidDriver = setMobileAppDriverManage(new AndroidDriver(new URL(Prop.getProperty("mobile.cloud.private.url")), getOptions(options)));
              }
          } else {
              dataConfig.androidDriver = setMobileAppDriverManage(new AndroidDriver(new URL(Prop.getProperty("localGridHub")), getOptions(options)));

          }
      return dataConfig.androidDriver;
  }
